...
object Registration extends DelvingController {
  ...
  def register() = {
    ...
    if (Validation.hasErrors) {
      ...
      // TODO save the node
      val newUser = User(reference = UserReference(r.displayName, getNode, getUserId(r.displayName)), firstName = r.firstName, lastName = r.lastName, email = r.email, password = Crypto.passwordHash(r.password1), isActive = false, activationToken =Some(activationToken), resetPasswordToken = None)
      User.insert(newUser)
