...
object Repositories extends DelvingController {
  ...
  def listWriteableAsTokens(q: String): Result = {
    ...
    val repos = for(dataSet <- DataSet.findAllByOwner(getUserReference).filter(dataSet => dataSet.spec.toLowerCase contains (q))) yield Token(dataSet._id.toString, dataSet.name)
    Json(repos)
  }
