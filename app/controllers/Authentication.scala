...
class Authentication(implicit val bindingModule: BindingModule) extends ApplicationController {
  ...
  def logout = MultitenantAction {
    Redirect("/login").withNewSession.discardingCookies(DiscardingCookie(REMEMBER_COOKIE)).flashing(
        ...
        loginForm.bindFromRequest.fold(
          ...
          user => {
            ...
            HubUser.dao.findByUsername(userName).orElse {
              ...
              Redirect("/login").flashing(("error", "Sorry, something went wrong while logging in, please try again"))
            }
          }
        )
    }
