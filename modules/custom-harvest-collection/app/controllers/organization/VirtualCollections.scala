          ...
          virtualCollectionForm => {
            ...
            virtualCollectionForm.id match {
              case Some(id) =>
                VirtualCollection.dao.findOneById(id) match {
                  case Some(vc) =>
                    ...
                    VirtualCollection.dao.createVirtualCollectionFromQuery(id, virtualCollectionQuery.toSolrQuery, Some(connectedUser)) match {
                      case Right(u) =>
              ...
              case None =>
                ...
                id match {
                  case Some(vcid) =>
                    VirtualCollection.dao.createVirtualCollectionFromQuery(vcid, virtualCollectionQuery.toSolrQuery, Some(connectedUser)) match {
                      case Right(ok) => Ok
                      case Left(t) =>
                        logError(t, "Error while computing virtual collection")
                        Error("Error computing virtual collection")
