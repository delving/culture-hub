  ...
  */
  ...
  override val routes: ListMap[(String, Regex), (List[String], Map[String, String]) => Handler] = ListMap(
    ("GET", """^/organizations/([A-Za-z0-9-]+)/virtualCollection$""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.list(pathArgs(0))
    },
    ("GET", """^/organizations/([A-Za-z0-9-]+)/virtualCollection/add$""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.virtualCollection(pathArgs(0), None)
    },
    ("GET", """^/organizations/([A-Za-z0-9-]+)/virtualCollection/([A-Za-z0-9-]+)/update$""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.virtualCollection(pathArgs(0), Some(pathArgs(1)))
    },
    ("POST", """^/organizations/([A-Za-z0-9-]+)/virtualCollection/submit$""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.submit(pathArgs(0))
    },
    ("GET", """^/organizations/([A-Za-z0-9-]+)/virtualCollection/([A-Za-z0-9-]+)""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.view(pathArgs(0), pathArgs(1))
    },
    ("DELETE", """^/organizations/([A-Za-z0-9-]+)/virtualCollection/([A-Za-z0-9-]+)""".r) -> {
      (pathArgs: List[String], queryString: Map[String, String]) => controllers.organization.VirtualCollections.delete(pathArgs(0), pathArgs(1))
    }
  )
...
object CustomHarvestCollectionPlugin {
  ...
  val CUSTOM_HARVEST_COLLECTION_KEY = "custom-harvest-collection"
