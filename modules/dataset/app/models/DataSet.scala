...
case class DataSet(
  ...
  def getRecords(
    ...
    // always filter on metadataFormat in the query and not afterwards because that can give empty results
    val records = cache.listByMetadataPrefix(position, Some(limit), from, until, Some(metadataFormat)) // .filter(_.xml.contains(metadataFormat))
