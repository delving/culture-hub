...
case class DataSet(
    ...
    def getAllMappingSchemas: Seq[Schema] = mappings.map(mapping =>
        Schema(mapping._2.schemaPrefix, mapping._2.schemaVersion)
    ).toSeq.distinct

    def getPublishableMappingSchemas = getAllMappingSchemas.filter(
        schema =>
            formatAccessControl.get(schema.prefix).isDefined
    ).filter(
        schema =>
            formatAccessControl(schema.prefix).isPublicAccess
                || formatAccessControl(schema.prefix).isProtectedAccess
    ).toList
